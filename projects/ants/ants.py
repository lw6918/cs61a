...
class Ant(Insect):
    ...
    def buff(self):
        ...
        #     self.damage *= 2
        #     self.buffed = True
        # elif self.is_container == True and self.ant_contained != None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        range = 0
        while p.is_hive == False:
            if p.bees and self.min_range <= range <= self.max_range:
                return random_bee(p.bees)
            p = p.entrance
            range += 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        #         self.place.bees[self.place.bees.index(b)].reduce_health(amount+self.damage)
        #     else:
        #         self.place.bees[self.place.bees.index(b)].reduce_health(amount)
        # super().reduce_health(amount)
        for b in self.place.bees[:]:    # b和bees中的bee为同一对象
            damage = amount + (self.damage if amount >= self.health else 0)
            b.reduce_health(damage)
        super().reduce_health(amount)
        # END Problem 5
