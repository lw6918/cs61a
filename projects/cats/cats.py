...
def accuracy(typed, reference):
    ...
    total=0.0
    if typed_len == reference_len == 0:
        return 100.0
    elif typed_len == 0 and reference_len != 0:
        return 0.0
    elif typed_len !=0 and reference_len == 0:
        return 0.0
    else:
        for i in range(0,min(typed_len,reference_len)):
            if typed_words[i] == reference_words[i]:
                total += 1.0
    return total / typed_len *100
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    #     distance = diff_function(typed_word, item, limit)
    ...
    if typed_word in valid_words:
        return typed_word
    min_item = min(valid_words, key = lambda word: diff_function(typed_word, word, limit))
    if diff_function(typed_word, min_item, limit) > limit:
        return typed_word
    else:
        return min_item
    # END PROBLEM 5
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    count = 0
    for i in range(sofar_len):
        if sofar[i] == prompt[i]:
            count += 1
        else:
            break
    upload({'id': user_id, 'progress': count / prompt_len})
    return count / prompt_len
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    p_len = len(times_per_player)
    i = 0
    times = list([] for k in range(p_len) if k < p_len)
    while i < p_len:
        j_len = len(times_per_player[i])
        times[i] = [times_per_player[i][j+1] - times_per_player[i][j] for j in range(j_len) if j < j_len-1]
        i += 1
...
def fastest_words(match):
    ...
    for w in word_indices:
        ...
        for p in player_indices:
            if time(match,p,w) < min:
                min = time(match,p,w)
                min_i = p
        fastest[min_i].append(word_at(match,w))
    return  fastest
    # END PROBLEM 10
