...
def balanced(m):
    ...
    assert is_mobile(m)
    def helper(m):
        m_left_end = end(left(m))
        m_right_end = end(right(m))
        m_left_weight = 0
        m_right_weight = 0
        if is_planet(m_left_end):
            m_left_weight = size(m_left_end)
        else:
            m_left_weight = helper(m_left_end)
        if is_planet(m_right_end):
            m_right_weight = size(m_right_end)
        else:
            m_right_weight = helper(m_right_end)
        if m_left_weight == 0 or m_right_weight == 0:
            return 0
        if m_left_weight * length(left(m)) == m_right_weight * length(right(m)):
            return  m_left_weight + m_right_weight
        return 0
    if helper(m):
        return True
    return False
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        m_label = label(left_branch) + label(right_branch)
        return tree(m_label, [left_branch, right_branch])
    # answer:
    # if is_planet(m):
    #     return tree(size(m))
    # else:
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) > 0 and lower_bound(y) > 0, 'AssertionError'
